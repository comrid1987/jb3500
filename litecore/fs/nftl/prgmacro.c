...
unsigned long sectorrand[((MAX_BLOCK-MAX_FREE_BLOCK)*MAX_PAGE_PER_BLOCK)*MLAYER_SETS];
...
//      rt_kprintf("error at page %l: exp %lx got %lx offset %lx\n", sector, sector, lptr[0], lptr[1]);
        return 1;
    }
    return 0; //ok
}
...
//	rt_memset (prg_pfevents,0,sizeof(prg_pfevents));
//	rt_memset (prg_ffevents,0,sizeof(prg_ffevents));
