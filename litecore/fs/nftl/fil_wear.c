...
unsigned long gl_dynamic_wear_info[MAX_FREE_BLOCK]; //dynamic wear info is here       //用于在程序运行过程中记录空闲块的wear
static unsigned char gl_dynamic_lock[(MAX_FREE_BLOCK+7) >> 3];//bitfield for locking mechanism //每位代表其对应的空闲块是否在使用。1：使用，0：空闲
                                                              //标示空闲块中暂时的非空闲块
...
typedef struct {
	...
	unsigned long wear;
} STATIC_WEAR_INFO;  //用于缓存当前block上wear最小的8个逻辑block

#define MAXSTATICWEAR 8    //maximum deep of static wear leveling

static STATIC_WEAR_INFO gl_static_wear_info[MAXSTATICWEAR];

static unsigned long gl_static_cnt;	 //counter for static wearing
...
void wear_init() {   //初始化wear全局变量
unsigned short a;
   for (a=0; a<MAXSTATICWEAR; a++) {  
      gl_static_wear_info[a].logblock=BLK_NA;
      gl_static_wear_info[a].wear=WEAR_NA;
   }
   ...
   gl_static_cnt=2; //wait cycle after power on
...
unsigned long static_wear=WEAR_NA;
   if (!gl_static_cnt) {
      //在gl_static_wear_info[8]中找出wear最小的logblock
      for (a=0; a<MAXSTATICWEAR; a++) {  
         unsigned long wear=gl_static_wear_info[a].wear;
         unsigned short logblock=gl_static_wear_info[a].logblock;
         ...
         for (b=0; b<MAX_FREE_BLOCK; b++) {   //search for limit
			if (!( (*dynamic_lock) & (*lookup_mask) ) ) { //check if locked
				if (!((*free_table) & FT_BAD)) {	//if it is realy free
            				unsigned long wear=gl_dynamic_wear_info[b];
            				...
            				if (wear!=WEAR_NA) {
             					  if (wear>static_wear+WEAR_STATIC_LIMIT) {
                  					...
                  					gl_static_cnt=WEAR_STATIC_COUNT;
