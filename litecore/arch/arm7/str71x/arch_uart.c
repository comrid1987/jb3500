...
static UART_TypeDef * const str7_tblUartId[] = {UART0, UART1, UART2, UART3};
...
//
//-------------------------------------------------------------------------
void arch_UartIoHandler(uint_t nId)
{
	UART_TypeDef *pUart = str7_tblUartId[nId];
	uint8_t arrTemp[16], *pTemp = arrTemp;
	int i;
	...
	while (pUart->SR & UART_RxBufFull) {
		*pTemp++ = pUart->RxBUFR;
		if (pTemp >= ARR_ENDADR(arrTemp)) {
			...
			dque_Push(dqueue, p->parent->id | UART_DQUE_RX_CHL, arrTemp, sizeof(arrTemp));
#endif
			pTemp = arrTemp;
		}
	}
...
#elif IO_BUF_TYPE == BUF_T_DQUEUE
		dque_Push(dqueue, p->parent->id | UART_DQUE_RX_CHL, arrTemp, pTemp - arrTemp);
#endif
	}
