...
void rt_hw_serial_isr(rt_device_t device)
{
	...
	if (USART_GetITStatus(uart->uarht_device, USART_IT_TC) != RESET)
	{
		/* clear interrupt */
...
static void rt_hw_console_init()
{
	...
	USART_ClockInit(CONSOLE_USART, &USART_ClockInitStructure);

	/* enable interrupt */
	USART_ITConfig(CONSOLE_USART, USART_IT_RXNE, ENABLE);

#ifdef RT_USING_DEVICE
	rt_hw_serial_register(&uart1_device, "uart1", RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_STREAM, &uart1);
#endif
}
