...
uint16_t nrsec3000_RcvLEN(p_nrsec3000 p, uint8_t tbufLen,uint8_t *rbuf)
{
	...
	if(spi_TranChar(p->spi, TR_INS,&rbuf[0])== SYS_R_OK){
		if(tbufLen==1)
			return rbuf[0];
		else if(tbufLen==2){
			if(spi_TranChar(p->spi, TR_INS,&rbuf[1])== SYS_R_OK){
				if(rbuf[0]!=0xFF)
...
sys_res nrsec3000_RcvData(p_nrsec3000 p,uint8_t *rbuf,uint8_t rbufLen)
{
	...
	for(i=0;i<rbufLen;i++){
		res = spi_TranChar(p->spi,TR_INS,&rbuf[i]); 
		if(res!=SYS_R_OK)
			break;
	}
	return res;
...
//	int cnt = 1000;
	...
	/* receive state word */
	for(;cnt>0;cnt--){
		spi_TranChar(p->spi,TR_INS,&rbuf[0]);
		if(rbuf[0] == 0x90){
			spi_TranChar(p->spi,TR_INS,&rbuf[0]);
			if(rbuf[0] == 0x00){
				return SYS_R_OK;
...
//SAM1 导入对称密钥key
sys_res nrsec3000_SM1ImportKey(p_nrsec3000 p,uint8_t *key)
{
	uint8_t tbuf[32];
    uint8_t rbuf[32];
	uint8_t nCRC;
...
//SAM1 导入初始向量 IV
sys_res nrsec3000_SM1ImportIV(p_nrsec3000 p,uint8_t *IV)
{
	uint8_t tbuf[32];
    uint8_t rbuf[32];
	uint8_t nCRC;
...
//SM3 Hash运算并附带功能
sys_res nrsec3000_SM3(p_nrsec3000 p,uint8_t *buf,uint16_t bufLen,uint8_t *hash,
						uint8_t *pubkey,uint8_t *pucID,uint_t idLen)
{
	uint_t nLen,nELen;
	sys_res res;
	uint8_t tbuf[1024];
