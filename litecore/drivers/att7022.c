...
sys_res att7022_Reset(p_att7022 p, p_att7022_cali pCali)
{
	...
	att7022_WriteReg(p, ATT7022_REG_HFConst, 114);	//ÉèÖÃHFConst
#elif ATT7022_CONST_EC == 800
	att7022_WriteReg(p, ATT7022_REG_HFConst, 456);	//ÉèÖÃHFConst
#else
	...
	att7022_WriteReg(p, ATT7022_REG_GCtrlT7Adc, 0); 		//¹Ø±ÕÎÂ¶ÈºÍADC7²âÁ¿ 	
	att7022_WriteReg(p, ATT7022_REG_EnlineFreq, 0x007812);	//»ù²¨/Ğ³²¨²âÁ¿¹¦ÄÜ
	att7022_WriteReg(p, ATT7022_REG_EnHarmonic, 0x0055AA);	//»ù²¨/Ğ³²¨²âÁ¿¹¦ÄÜ
...
//------------------------------------------------------------------------
//Ãû	³Æ: BSP_ATT7022B_Phase_gainCalibration () 
//Éè	¼Æ: 
//Êä	Èë: -
//Êä	³ö: -
//·µ	»Ø: -
//¹¦	ÄÜ: ÏàÎ»½ÇĞ£Õı,¹¦ÂÊÒòÊıÎª  0.5L  µÄÌõ¼şÏÂ
//------------------------------------------------------------------------
...
uint32_t att7022_PhaseCali1Seg(p_att7022 p) {
	uint32_t phv;
	uint_t i;
	phv= att7022_PhaseCalibration(p,PHASE_A, 1); //7.5A´¦Ğ£Õı
	att7022_WriteEnable(p); //ATT7022BĞ£ÕıÊ¹ÄÜ
	for ( i = 0; i < 3; i++ )
	{
	...
	att7022_WriteDisable(p); //ATT7022BĞ£Õı½ûÖ¹
	return phv;
}
//------------------------------------------------------------------------
...
float att7022_FPhaseCaliData(p_att7022 p,uint8_t nPhase, uint8_t cali_point) 
{
	...
	eck = 3200.0f / (float)ATT7022_CONST_EC; //Âö³åÊä³öÏµÊı
	pcali_value = PCALI_CONST * 0.5; //phiconst_tab[cali_point];				//ÔØÈëĞ£ÕıµãµÄÓĞ¹¦¹¦ÂÊ³£Êı
	att7022_pvalue = att7022_ReadReg(p,ATT7022_REG_PA + nPhase); //¶ÁÈ¡ÓĞ¹¦¹¦ÂÊÖµ
